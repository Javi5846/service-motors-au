        ...
        <div className="flex items-center justify-between py-3">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            {isAdmin && (
              <Link
                href="/admin"
                className="text-[#DC2626] hover:text-[#B91C1C] text-base font-bold transition-all duration-200 hover:scale-105 inline-block"
              >
                Admin
              </Link>
            )}
        ...
        <div className="md:hidden bg-white border-t border-gray-200 px-4 py-4 space-y-3">
          ...
          {isAdmin && (
            <Link
              href="/admin"
              onClick={() => setIsOpen(false)}
              className="block text-[#DC2626] font-bold py-2 border-b border-gray-100"
            >
              Admin
            </Link>
          )}
